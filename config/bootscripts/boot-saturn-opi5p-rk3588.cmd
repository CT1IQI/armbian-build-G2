...
# net.ifnames=0 prevents renaming of the ethernet interfaces 
setenv extraargs "${extraargs}" devtype="${devtype}" devnum="${devnum} net.ifnames=0 "

if test "${check4usb}" = "true"; then
	setenv extraargs "${extraargs}" check4usb=true
fi
...
setenv bootargs \
root="${rootdev}" rootwait rootfstype="${rootfstype}" "${consoleargs}" consoleblank=0 \
loglevel=${verbosity} ubootpart="${partuuid}" usb-storage.quirks="${usbstoragequirks}" \
